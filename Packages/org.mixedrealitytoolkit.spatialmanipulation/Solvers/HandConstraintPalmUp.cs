        ...
        protected override bool IsValidController(XRNode? hand)
        {
            using (IsValidControllerPerfMarker.Auto())
            {
                    ...
                    XRSubsystemHelpers.HandsAggregator.TryGetJoint(TrackedHandJoint.Palm, hand.Value, out HandJointPose palmPose))
                {
                    float dotProduct;
                    #if PICO_INSTALL
                    dotProduct = Vector3.Dot(hand == XRNode.LeftHand ? -palmPose.Up : palmPose.Up, Camera.main.transform.forward);
                    #else
                    dotProduct = Vector3.Dot(palmPose.Up, Camera.main.transform.forward);
                    #endif
